...
/*    @Query("select new Post(p.id, p.title, p.teaser, p.feature, p.created, p.likes, p.category) from Post p where p.category = :category")*/
    ...
    @Query(value="SELECT i FROM Image i INNER JOIN Album a ON i.album = a WHERE i.isDelete = 0 AND a.isDelete = 0 and a.isPublic = 1")
    Page<Image> findAllByDeleteAndAlbumDeleteAndAlbumPublic(Pageable pageable);
